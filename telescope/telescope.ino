#define STEPER_EN       8
#define DIRRECTION_X    5
#define DIRRECTION_Y    6
#define DIRRECTION_Z    7
#define STEP_PULSE_X    2
#define STEP_PULSE_Y    3
#define STEP_PULSE_Z    4
#define LIMIT_X         9
#define LIMIT_Y         10
#define LIMIT_Z         11
#define LED_PIN         13
...
#define MILLS_BT_STEPS  10

void setup() { 

  Serial.begin(9600);

  pinMode(DIRRECTION_X, OUTPUT);
  pinMode(STEP_PULSE_X, OUTPUT);
  pinMode(DIRRECTION_Y, OUTPUT);
  pinMode(STEP_PULSE_Y, OUTPUT);

  pinMode(STEPER_EN, OUTPUT);
  pinMode(LED_PIN, OUTPUT);

  digitalWrite(LED_PIN, LOW);
  digitalWrite(STEPER_EN, LOW);
...
void rotate_axis(char axis, char dir, int steps){
  //Rotate X AXIS
  if (axis == 'X'){
    dir == 'R' ? digitalWrite(DIRRECTION_X, LOW) : digitalWrite(DIRRECTION_X, HIGH);
    Serial.println("MOTOR X");
    for (int n=0; n<= steps; n++){
      digitalWrite(STEP_PULSE_X, LOW);
      delay(MILLS_BT_STEPS);
      digitalWrite(STEP_PULSE_X, !digitalRead(STEP_PULSE_X));
    }

  }
  //Rotate Y AXIS
  if (axis == 'Y'){
    dir == 'R' ? digitalWrite(DIRRECTION_Y, LOW) : digitalWrite(DIRRECTION_Y, HIGH);
    Serial.println("MOTOR Y");
    for (int n=0; n<= steps; n++){
      digitalWrite(STEP_PULSE_Y, LOW);
      delay(MILLS_BT_STEPS);
      digitalWrite(STEP_PULSE_Y, !digitalRead(STEP_PULSE_Y));
    }
  }
}

void disable_motors(){
  digitalWrite(STEPER_EN, HIGH);
}
